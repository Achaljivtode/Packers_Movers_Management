...
function Dashboards() {
  ...
  const [image, setImage] = useState("/Resources/images/default.jpg");
  ...
  console.log(userRole ? userRole.toUpperCase() : "No Role Found");

  return (
    <div>
      <h1 className="text-3xl font-bold text-orange-400 bg-gray-300 p-4 text-center">
      ...
      <div className="flex flex-row justify-between my-10 p-10 lg:w-[1450px] mx-auto">
        <ul className=" w-[50%]">
          {Object.keys(Data).map((key) => (
            <li
              key={key}
              className="border p-2 my-5 w-[80%] mx-auto text-center bg-orange-400 text-white text-2xl font-semibold rounded-xl"
            >
              {key === "Logout" ? (
                <a
                  href={Data[key].link}
                  onClick={() => localStorage.setItem("role", "")}
                >
                  {key}
                </a>
              ) : (
